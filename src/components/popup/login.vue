    ...
    >
      <div v-if="isQRCode" class="content">
        ...
        <div class="hk-login-box-switch padding-b-45">
          <el-button
            class="hk-el-button"
            :class="ordinaryLogin == true ? 'is-focuse' : ''"
            @click="loginMode(true,'accountNumberLoginForm')"
          >密码登录</el-button>
          <el-button
            class="hk-el-button"
            :class="ordinaryLogin == false ? 'is-focuse' : ''"
            @click="loginMode(false,'phoneLoginForm')"
          >验证码登陆</el-button>
        </div>
        <el-form
          ref="accountNumberLoginForm"
          :model="accountNumberLoginForm"
          :rules="rules"
          label-width="110px"
          v-if="ordinaryLogin"
          class="form"
        >
          ...
          <el-form-item>
            ...
            <el-button
              type="primary"
              class="login-button"
              @click="onLogin('accountNumberLoginForm')"
            >登陆</el-button>
        ...
        >
          ...
          <el-form-item label="输入验证码" prop="code">
            <el-input v-model="phoneLoginForm.code" placeholder="请输入验证码">
              <div slot="suffix" class="code" @click="onCode">获取验证码</div>
            </el-input>
          </el-form-item>
          <el-form-item>
            <div @click="onRetrievePwd" class="forget-pwd">忘记密码</div>
            <el-button
              type="primary"
              class="login-button"
              @click="onPhoneLogin('phoneLoginForm')"
            >登陆</el-button>
            <div class="register" @click="onRefister">立即注册</div>
          </el-form-item>
        </el-form>
...
export default {
  ...
  methods: {
    ...
    getWechatPollingLogin() {
      wechatPollingLogin(this.uu_str)
        .then(res => {
          if (res.status == 200) {
            this.reload();
          }
            ...
            .then(res => {
              if (res.status === 200) {
                store.commit("LOGIN", {
                  ...
                  expires_time: dayjs(res.data.expires_time)
                });
                this.$emit("login");
              } else {
                this.$message({
