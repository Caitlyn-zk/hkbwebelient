<template>
  <div class="addproject" v-loading="loading">
    <div class="content hk-addproject-bar">
      <div class="hk-addproject-title-mbar">项目中心/{{ ruleForm.id == 0 ? '新增环评公示': '编辑环评公示'}}</div>
        ...
        >
          ...
          <el-form-item label="上传文件:">
            ...
            >
              ...
              <div slot="tip" class="el-upload__tip">只能上传docx/pdf文件，且不超过30M</div>
            </el-upload>
          </el-form-item>
          <div class="hk-cont-main-bar clearfix padding-l-34">
            ...
            <span
              class="hk-addproject-warning-bottm fr padding-r-80 hk-cursor"
              @click="onViewExamples"
            >查看示列</span>
          </div>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        detail: [{ required: true, message: "名字不能为空", trigger: "blur" }]
      }
    };
  },
  computed: {
    ...
    addPublici(ruleForm) {
      this.$refs[ruleForm].validate(valid => {
        ...
        if (valid) {
          ...
          addProjectUserNoticePublic(parameter)
            .then(res => {
              if (res.status === 200) {
                this.$message({
                  message: res.msg,
