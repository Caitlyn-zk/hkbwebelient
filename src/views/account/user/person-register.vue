  ...
  <div class="hk-login-main">
    <div class="hk-login-form clearfix">
      <div class="hk-login-right fr">
        <div class="hk-login-title-right">个人端-注册</div>
        <el-form v-if="isQRCode" class="hk-login-box hk-form hk-form-register" :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="100px">
          ...
          <div class="">
            <span class="font-16 text-line">已有账号！</span><router-link to="/account/login"><span class="font-16 text-green hk-cursor">去登录</span></router-link>
          </div>
        </el-form>
...
import {handlePersonLoginSuccess,toLogin} from '@/libs/login'
...
export default {
  ...
  methods: {
    ...
    // 获取cookie中的时间
    getCookieTime () {
      let getCookieTime = cookie.get('registerCodeTime')
      if (getCookieTime !== null) {
        this.isCookie = getCookieTime
        this.registerCodeTime = false
      } else {
        this.isCookie = null
      }
    },
    ...
    countDown (times) {
      ...
      function countDownStart () {
        ...
        count++
        cookie.set('registerCodeTime', times - count + 1)
